...
export function ConciliacionPagos({
  ...
  async function registrar() {
    ...
    const montoLeido = monto.trim() ? leerMonto(monto) : null
    if (monto.trim() && montoLeido == null) {
      setError("No entendimos el monto. Escríbelo como sale en el banco, por ejemplo 27.388,87.")
      return
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Después de la lista: lo que se viene a hacer al abrir el aviso es
          confirmar los que esperan, no registrar uno nuevo. */}
      <section className="rounded-2xl border border-gray-200 bg-white p-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <input
            ...
            className="h-12 min-w-40 flex-1 rounded-xl border border-gray-200 bg-white px-3 text-base tabular-nums text-gray-900 outline-none placeholder:text-gray-500 focus:border-emerald-500"
          />
          <input
            value={monto}
            ...
            className="h-12 w-36 rounded-xl border border-gray-200 bg-white px-3 text-base tabular-nums text-gray-900 outline-none placeholder:text-gray-500 focus:border-emerald-500"
          />
          <button
            type="button"
