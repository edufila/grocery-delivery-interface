-- 0052 · Avisos y entrega programada
--
-- Con la 0051 un pedido puede ser para el jueves. Pero avisos_pendientes lo
-- contaba como "pedido pagado esperando shopper" desde que se pagaba: a los
-- shoppers les sonaba hoy por algo que hay que comprar en dos días, y
-- terminaban tomándolo antes de tiempo o dejando de hacerle caso al aviso.
--
-- Igual que la 0047 con dos cambios:
--   · los disponibles cuentan solo si son lo antes posible o si falta menos de
--     una hora para su turno (lo mismo que ordena la lista del shopper);
--   · al cliente con pedido programado, "Confirmamos tu pago" le dice para
--     cuándo quedó.
--
-- Necesita la 0051. Correr entero en Supabase → SQL Editor. Es idempotente.
...
begin
  ...
  if v_rol in ('shopper', 'admin', 'dev') then
    ...
    where shopper_id is null
      ...
      -- Un programado para más tarde todavía no hay que salir a comprarlo.
      and (entregar_desde is null or entregar_desde < now() + interval '60 minutes');
  end if;
  ...
  select code, status, shopper_id, payment_verified_at, entregar_desde
    into v_mio
  from public.orders
  ...
  if found then
    ...
    elsif v_mio.status = 'confirmado' and v_mio.payment_verified_at is not null
          and v_mio.entregar_desde is not null then
      return jsonb_build_object(
        'titulo', 'Confirmamos tu pago',
        'cuerpo', 'Tu pedido quedó programado para el ' ||
          to_char(v_mio.entregar_desde at time zone 'America/Caracas', 'DD/MM') || ' a las ' ||
          lower(to_char(v_mio.entregar_desde at time zone 'America/Caracas', 'FMHH12:MI AM')) || '.',
        'url', '/pedidos/' || v_mio.code
      );
